...
import { LIVE_MAP_ID, Mapcn } from "@/components/ui/map";
...
async function fetchLocationsFallback(): Promise<LiveLocation[]> {
  ...
  const { data: logs, error: logsError } = await supabase
    .from("location_logs")
    .select(
      "user_id, latitude, longitude, recorded_at, profiles:user_id (full_name)",
    )
  ...
  attendance?.forEach((att: any) => {
    if (!locationMap.has(att.user_id)) {
      const profile = Array.isArray(att.profiles)
        ? att.profiles[0]
        : att.profiles;
...
function LiveMapInner({
  ...
  const fitAllLocations = useCallback(() => {
    ...
    if (!m) return;
    const withCoords = locationsRef.current.filter(
      (l) => l.status !== "offline",
    );
  ...
  const geocodeLocations = useCallback(async (locs: LiveLocation[]) => {
    ...
    const hasCachedNames = updated.some(
      (l, i) => l.placeName && !locs[i].placeName,
    );
    if (hasCachedNames) {
  ...
  useEffect(() => {
    ...
    async function load(isInitial: boolean) {
      ...
      const hasNewData =
        !cached?.length ||
        allLocations.length !== cached.length ||
        allLocations.some(
          (loc, i) => loc.recorded_at !== cached[i]?.recorded_at,
        );
            ...
            >
              <span
                style={{
                  ...
                  gap: 4,
                  background:
                    selectedUser.status === "live" ? "#ecfdf5" : "#fef3c7",
              ...
              >
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      selectedUser.status === "live" ? "#10b981" : "#f59e0b",
